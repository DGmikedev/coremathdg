...
/**
 * Check the data and return true if all right
 * 'nxn'  => arrn[n][n] 
 * 'num'  => any real number 
 * 'vecN' => number[real number ]
 * @param data { any:[][] }
 * 
 * @returns  { status, msg, res }
 *//*
export function checkAllDataIn(data:any[][]):Result{

    let f_result: Result = result(false, 'Some data was not analized verify this type of data ', NaN);

    data.forEach((vectorData)=>{

        if( vectorData[0] == 'nxn' ){

                f_result = iScudrticAndNumc(vectorData[1])

                if(!f_result.status) return result( f_result.status, f_result.msg, f_result.res )    

        
        }else if( vectorData[0] == 'num' ){

                f_result = iSNumeric(vectorData[1])

                if(!f_result.status) return result( f_result.status, f_result.msg, f_result.res )

        }else if( vectorData[0] == 'vecN' ){

                f_result = iSNumVect(vectorData[1])

                if(!f_result.status) return result( f_result.status, f_result.msg, f_result.res )

        }else{ return result( f_result.status, f_result.msg, f_result.res ) }
    })

    return result(f_result.status, f_result.msg, f_result.res)
}
*/